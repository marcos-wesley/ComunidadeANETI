...
export class DatabaseStorage implements IStorage {
  ...
  async getGroupPosts(groupId: string, limit: number = 20, offset: number = 0): Promise<GroupPostWithDetails[]> {
    const posts = await db
      .select({
        ...
        isDeleted: groupPosts.isDeleted,
        deletedAt: groupPosts.deletedAt,
      ...
      posts.map(async (post) => {
        ...
        const [commentsCount] = await db
          ...
          .where(and(
            ...
            eq(groupPostComments.isDeleted, false)
          ));
        ...
        const comments = await db
          .select({
            ...
            replyToId: groupPostComments.replyToId,
            isDeleted: groupPostComments.isDeleted,
            deletedAt: groupPostComments.deletedAt,
            createdAt: groupPostComments.createdAt,
            updatedAt: groupPostComments.updatedAt,
            author: {
              id: users.id,
              fullName: users.fullName,
              username: users.username,
  ...
  async getGroupPost(postId: string): Promise<GroupPostWithDetails | undefined> {
    const [post] = await db
      .select({
        id: groupPosts.id,
        groupId: groupPosts.groupId,
  ...
  async deleteGroupPost(postId: string, userId: string): Promise<boolean> {
    const result = await db
      ...
      .where(and(
        eq(groupPosts.id, postId),
        eq(groupPosts.authorId, userId)
      ));

    return result.rowCount > 0;
  }

  // Group Post Likes Management
  async likeGroupPost(postId: string, userId: string): Promise<GroupPostLike> {
    const [like] = await db
  ...
  async unlikeGroupPost(postId: string, userId: string): Promise<boolean> {
    const result = await db
  ...
  async getGroupPostLike(postId: string, userId: string): Promise<GroupPostLike | undefined> {
    const [like] = await db
      .select()
      .from(groupPostLikes)
      .where(and(
  ...
  // Group Post Comments Management
  ...
  async getGroupPostComments(postId: string): Promise<(GroupPostComment & { author: Pick<User, 'id' | 'fullName' | 'username' | 'profilePicture'> })[]> {
    return await db
      .select({
        id: groupPostComments.id,
        postId: groupPostComments.postId,
  ...
  async deleteGroupPostComment(commentId: string, userId: string): Promise<boolean> {
    const result = await db
      .update(groupPostComments)
      .set({
        isDeleted: true,
        deletedAt: new Date(),
        updatedAt: new Date(),
      ...
      .where(and(
        eq(groupPostComments.id, commentId),
        eq(groupPostComments.authorId, userId)
      ));
