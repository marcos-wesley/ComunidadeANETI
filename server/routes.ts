...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/groups/:groupId/posts", isAuthenticated, async (req, res) => {
    try {
      ...
      if (group.visibility === 'private') {
        const user = await storage.getUser(userId);
        if (!user || (!user.planName && user.planName !== null)) {
          return res.status(403).json({
            success: false,
            message: "Acesso negado. Este é um grupo privado."
          });
        }

        const planLevel = user.planName?.toLowerCase();
        const allowedLevels = group.allowedPlanLevels?.map(level => level.toLowerCase()) || [];
        
        if (!allowedLevels.includes(planLevel || '')) {
          return res.status(403).json({
            success: false,
            message: "Acesso negado. Seu plano de associação não permite acesso a este grupo."
          });
        }
      }

      // Check if user is a member (for both public and private groups)
      const membership = await storage.getGroupMembership(groupId, userId);
      if (!membership || !membership.isActive) {
        return res.status(403).json({
          success: false,
          message: "Você precisa ser membro do grupo para ver as publicações."
        });
      }

      const posts = await storage.getGroupPosts(groupId, parseInt(limit as string), parseInt(offset as string));
      res.json(posts);
    } catch (error) {
  ...
  app.post("/api/groups/:groupId/posts", isAuthenticated, async (req, res) => {
    try {
      const { groupId } = req.params;
      const userId = req.user.id;
      const { content, imageUrl, attachmentUrl, attachmentName, postType } = req.body;
      ...
      if (!membership || !membership.isActive) {
        return res.status(403).json({
          success: false,
          message: "Você precisa ser membro do grupo para publicar."
        });
      }

      // Check if user can post (only moderators and admins can post in group feeds)
      const user = await storage.getUser(userId);
      const isAdmin = user?.role === 'admin';
      const isModerator = membership.role === 'moderator' || group.moderatorId === userId;
      
      if (!isAdmin && !isModerator) {
        return res.status(403).json({
          success: false,
          message: "Apenas moderadores e administradores podem publicar no feed do grupo."
        });
      }
      ...
      const post = await storage.createGroupPost(postData);
      const postWithDetails = await storage.getGroupPost(post.id);
  ...
  app.get("/api/groups/:groupId/posts/:postId", isAuthenticated, async (req, res) => {
    try {
  ...
  app.delete("/api/groups/:groupId/posts/:postId", isAuthenticated, async (req, res) => {
    try {
      ...
      // Check if user can delete the post (author, group moderator, or admin)
      const post = await storage.getGroupPost(postId);
      if (!post || post.groupId !== groupId) {
        return res.status(404).json({ success: false, message: "Publicação não encontrada" });
      }
  ...
  app.post("/api/groups/:groupId/posts/:postId/like", isAuthenticated, async (req, res) => {
    try {
      ...
      const existingLike = await storage.getGroupPostLike(postId, userId);
      
      if (existingLike) {
        // Unlike the post
        await storage.unlikeGroupPost(postId, userId);
        res.json({ success: true, liked: false, message: "Curtida removida" });
      } else {
        // Like the post
        await storage.likeGroupPost(postId, userId);
        res.json({ success: true, liked: true, message: "Publicação curtida" });
      }
    } catch (error) {
      console.error("Error liking/unliking group post:", error);
      res.status(500).json({ success: false, message: "Erro interno do servidor" });
    }
  });
  ...
  app.post("/api/groups/:groupId/posts/:postId/comments", isAuthenticated, async (req, res) => {
    try {
      ...
      const comment = await storage.createGroupPostComment(commentData);
      
      // Return comment with author info
      const [commentWithAuthor] = await storage.getGroupPostComments(postId);
      const newComment = commentWithAuthor ? 
        (await storage.getGroupPostComments(postId)).find(c => c.id === comment.id) : null;
  ...
  app.delete("/api/groups/:groupId/posts/:postId/comments/:commentId", isAuthenticated, async (req, res) => {
    try {
      ...
      // Check if user can delete the comment (author, group moderator, or admin)
      const comments = await storage.getGroupPostComments(postId);
      ...
      const deleted = await storage.deleteGroupPostComment(commentId, userId);
      if (!deleted) {
        return res.status(500).json({
          success: false,
